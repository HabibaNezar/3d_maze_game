...
class Monster:
    ...
    def collission_1(self, cam):
        global coins_result # that mean it affect ihe coin result only and when we remove it we lose when we toush thw monister
        if cam.camera_pos[0] + 0.6 > self.x and cam.camera_pos[0] - 0.6 < self.x:  #checks if the camera and the monster are horizontally aligned within a certain range
            if cam.camera_pos[2] + 0.6 > self.z and cam.camera_pos[2] - 0.6 < self.z: # checks if the camera andthe monster are vertically aligned within a certain range
                return True  # return true if the collision happened
